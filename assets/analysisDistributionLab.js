...
const DIST_KEYS = ['industry', 'sub_industry', 'programme_positioning', 'membership_type', 'geographic_scope', 'country', 'target_customer', 'access_registration'];
...
export function mount(container, ctx) {
  ...
  container.innerHTML = `
    ...
    <div class="control-row">
      ...
      <div class="control-group"><label>Chart Type</label>
        <select class="control-select" id="dist-charttype"><option value="bar">Horizontal Bar</option><option value="donut">Donut</option></select>
      </div>
  ...
  [dimSel, typeSel, measureSel].forEach(sel => sel.addEventListener('change', () => {
    state.dimKey = dimSel.value; state.chartType = typeSel.value; state.measure = measureSel.value;
  ...
  function applyConfig(config) {
    ...
    dimSel.value = state.dimKey; typeSel.value = state.chartType; measureSel.value = state.measure;
    render(lastProgrammes);
  }
